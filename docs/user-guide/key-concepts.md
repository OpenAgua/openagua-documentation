...
The user does not need to deal with templates to get started with OpenAgua. However, as a model becomes more sophisticated, or if the core modeling logic is changed, OpenAgua comes with a [template editor](/user-guide/setting-up-model/#define-the-model-template), which may then be applied to a new or existing network as [described here](/user-guide/setting-up-model/#createedit-networks).
